...
void YoloObjectDetector::init()
{
  ...
  rclcpp::QoS camera_qos(cameraQueueSize);
...
void YoloObjectDetector::cameraCallback(const sensor_msgs::msg::Image::ConstSharedPtr & msg)
{
  ...
  try {
    // Accept both rgb8 and bgr8 encodings
    if (msg->encoding == "rgb8") {
      cam_image = cv_bridge::toCvCopy(msg, "rgb8");
      // Convert RGB to BGR for OpenCV
      cv::cvtColor(cam_image->image, cam_image->image, cv::COLOR_RGB2BGR);
    } else {
                                      ...
                                      int frames, int fullscreen)
{
  ...
  image **alphabet = NULL;  // Skip alphabet loading to avoid OpenCV compatibility issues
  demoNames_ = names;
...
{
  IplImage temp = cvIplImage(camImageCopy_);
  IplImage* ROS_img = new IplImage(temp);
